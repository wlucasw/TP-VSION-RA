...
bool detecte[16] = {false};
float taille[16] = {0.0f};
...
// Drawing function
void ArUco::drawScene(bool jeuEnCours) {
   ...
   for (unsigned int m=0 ; m<m_Markers.size() ; m++){
      ...
      // Drawing a cube
	  if (jeuEnCours) {glColor3f(0.5f,0.0f,1.0f);} //Cube en violet pendant le jeu
	  else{glColor3f(1.0f,0.0f,0.0f);} //Cube à la fin du jeu
      glTranslatef(0, 0, m_MarkerSize/2);
      
      glPushMatrix();
      glutSolidCube(taille[m_Markers[m].id]);
...
// Idle function
void ArUco::idle(Mat newImage, bool jeuEnCours) {
   // Getting new image
   ...
   if (jeuEnCours) {
	  cout << "JEU EN COURS" << endl << endl;
	   //Changer la taille du cube de chaque marqueur
	  for (unsigned int m=0 ; m<m_Markers.size() ; m++){
		 if (!detecte[m_Markers[m].id]){
		    taille[m_Markers[m].id] += m_MarkerSize/50;
		 }
	  }
	  //Changer l'etat de chaque marqueur (détecté à cette frame ou pas)
	  for (unsigned int i=0 ; i<16 ; i++){
		detecte[i] = false;
	  }
	  for (unsigned int m=0 ; m<m_Markers.size() ; m++){
		if (!detecte[m_Markers[m].id]){
		  detecte[m_Markers[m].id] = true;
		}
	  }
   }
   else { //Affichage des scores à la fin de la partie
	   for (unsigned int i=0 ; i<16 ; i++){
		   if (taille[i] != 0.0f){
			  cout << "MARQUEUR " << i << " : " << taille[i] *1000 << endl;
		   }
	   }
   }
